#### Servidor para Laboratório de Túnel do SSH (SSH Reverse Shell) ####
# Este script é o servidor que irá receber a conexão reversa do cliente (ssh_rcmd_reverse_shell.py). Ele utiliza a biblioteca Paramiko para criar um servidor SSH e aceitar conexões.
# O servidor escuta na porta 2222 e, quando um cliente se conecta, ele aceita a sessão e aguarda comandos do cliente. O cliente envia comandos para o servidor, que os executa e retorna a saída de volta para o cliente.
# Para testar, primeiro execute este script para iniciar o servidor, e depois execute o ssh_rcmd_reverse_shell.py para conectar ao servidor e obter a shell reversa.
